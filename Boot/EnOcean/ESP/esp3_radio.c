...
/**
  ...
  * @retval None
  */
uint8_t EnOcean_GetSendQueuePopID(uint16_t u16SendPeriod)
{
    uint8_t i;
    uint8_t j;
    uint8_t u8Status;
    ...
    if ( OK == u8Status )
    {
        ...
        EnOceanSetSendPeriod( u16SendPeriod );
    }
...
uint8_t EnOceanESP3SendStateProcess(void)
{
    ...
    else if ( eTX_SEND_OK == Esp3Tx.u8State )//shift to the wait response
    {
        ...
        {
            Esp3Tx.u16OvfCnt = 20000;
        }
        else
        {
            Esp3Tx.u16OvfCnt = 10000;
        }
    }
...
{
    EnOceanRun.u16SendPeriod = u16SendPeriod*10;
}
/**
  * @brief
  * @param
